...
This script processes all FITS files in test_directory/ to find peaks and create 64x64 cutouts.
All cutouts are saved to a single output folder: output_peaks_from_test_directory/
...
def process_single_diff_file(fits_path, output_dir, cutout_size=64, 
                            sigma_threshold=3.0, centering_method='centroid',
                            edge_buffer=32, max_peaks=None, allow_edge_peaks=True):
    """
    Process a single diff FITS file to find peaks and create cutouts.
    All cutouts are saved to a single flat output directory.
    ...
    output_dir : str or Path
        Output directory for FITS cutouts
    cutout_size : int
    ...
    try:
        ...
        for i, (peak_y, peak_x) in enumerate(peaks):
            ...
            # Create unique filename with source file name
            cutout_name = f"{file_name}_peak_{i:03d}_y{int(round(peak_y))}_x{int(round(peak_x))}_sigma{peak_sigma:.1f}.fits"
            fits_path_out = output_dir / cutout_name
            
            save_cutout_fits(cutout, fits_path_out, header)
            saved_count += 1
        
        print(f"\n✓ Saved {saved_count} cutouts from {fits_path.name}")
        return saved_count
...
def main():
    """
    Main function - process all FITS files in test_directory/ and output to single folder.
    """
    # ==== CONFIGURATION ====
    # Input directory containing all FITS files
    input_dir = Path("test_directory")
    
    # Output directory - single flat folder for all cutouts
    output_dir = Path("output_peaks_from_test_directory")
    
    # Detection parameters
    sigma_threshold = 3.5        # Lower = more sensitive
    edge_buffer = 32             # Pixels from edge to exclude
    ...
    cutout_size = 64            # Size of cutout boxes
    ...
    if not input_dir.exists():
        ...
        print("\nPlease create 'test_directory/' and add FITS files to it")
        return
    ...
    for idx, fits_file in enumerate(fits_files, 1):
        ...
        num_cutouts = process_single_diff_file(
            fits_file, 
            output_dir,
    ...
    print(f"  Total cutouts created: {total_cutouts}")
    print(f"  Average cutouts per file: {total_cutouts / successful_files if successful_files > 0 else 0:.1f}")
